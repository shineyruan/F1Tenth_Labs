...
void PurePursuitPlanner::pose_callback(const nav_msgs::Odometry::ConstPtr& odom_msg) {
    ...
    if (_currIdx < 0)
        findWaypoint(_position);

    findGoal(_position);

    Point goal_local = Point::toLocalFrame2D(_position, _waypoints[_currIdx], _orientation);
    double curvature = 2 * (goal_local.y) / (goal_local.distance2(Point()));

    double steering_angle = K_p * curvature;

    ROS_INFO_STREAM(_currIdx << " " << _position.y << " " << _waypoints[_currIdx].y << " " << curvature);
