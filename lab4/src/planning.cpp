...
{
    _n = ros::NodeHandle();
    _scanSubscriber = _n.subscribe("/scan", 1, &FollowGapPlanner::laserScan_callback, this);
    _steerPublisher = _n.advertise<ackermann_msgs::AckermannDriveStamped>("/nav", 1);
}

void FollowGapPlanner::laserScan_callback(const sensor_msgs::LaserScan::ConstPtr& scan_msg) {
    // 0. Save the current laser scan
    //      Truncate the field of view angle
    float fov_min = 0.f, fov_max = 0.f, angle_increment = 0.f;
    std::vector<float> scan_ranges = truncateFOV(scan_msg, &fov_min, &fov_max, &angle_increment);
    ...
    for (int i = 0; i < num_scan; ++i) {
        if (findRadius(scan_ranges[i], min_range, angle_increment * std::abs(min_idx - i)) <= _bubbleRadius)
            scan_ranges[i] = 0;
    }
    // imposes safety angle on the scan data
    imposeSafetyAngle(scan_ranges, static_cast<int>(toRadius(_safety_angle) / angle_increment));

    // 3. Find maximum length sequence of consecutive non-zeros among "free space" points
    //          In this case we only have 2 chunks: left chunk of the bubble/right chunk of the bubble
    unsigned int target_idx = findMaxFreeSpace(scan_ranges);

    // 4. Choose the furthest point in free space, set the steering angle towards it
    //
...
void FollowGapPlanner::imposeSafetyAngle(std::vector<float>& ranges, const int safety_angle_radius) {
    ...
    for (const auto& p : obstacle_regions) {
        for (int j = p.first; j >= std::max(0, p.first - safety_angle_radius); --j)
            ranges[j] = 0;
        for (int j = p.second; j < std::min(static_cast<int>(ranges.size()), p.second + safety_angle_radius); ++j)
            ranges[j] = 0;
    }
}

void FollowGapPlanner::processScan(std::vector<float>& ranges, const double range_min, const double range_max) {
    // clip noisy data
    for (float& r : ranges) {
        if (r < range_min)
            r = range_min;
...
unsigned int FollowGapPlanner::findMaxFreeSpace(const std::vector<float>& ranges) {
    // one-pass algorithm
    ...
    auto max_range_ptr = std::max_element(ranges.begin() + (max_end - max_count), ranges.begin() + max_end);
    return (max_range_ptr - ranges.begin());
}

std::vector<float> FollowGapPlanner::truncateFOV(const sensor_msgs::LaserScan::ConstPtr& scan_msg,
                                                 float* fov_min, float* fov_max, float* angle_increment) {
    // only keeps data between -_fov/2 to _fov/2
    double fov_2 = toRadius(_fov / 2);
    int fov_min_id = static_cast<int>(((-fov_2 - scan_msg->angle_min) / scan_msg->angle_increment));
    int fov_max_id = static_cast<int>((fov_2 - scan_msg->angle_min) / scan_msg->angle_increment);
