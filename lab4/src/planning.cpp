...
void FollowGapPlanner::laserScan_callback(
  ...
  for (int i = 0; i < num_scan; ++i) {
    if (findRadius(scan_ranges[i], min_range,
                   angle_increment * std::abs(min_idx - i)) <= _bubbleRadius)
      scan_ranges[i] = 0;
  }
  // imposes safety angle on the scan data
  imposeSafetyAngle(
      scan_ranges, static_cast<int>(toRadius(_safety_angle) / angle_increment));
...
unsigned int FollowGapPlanner::findMaxFreeSpace(
  ...
  auto max_range_ptr = std::max_element(ranges.begin() + (max_end - max_count),
                                        ranges.begin() + max_end);
